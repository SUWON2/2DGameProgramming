...
class TitleState:
    ...
    def update(self):
        ...
        if self.pressed_start_button is False and self.pressed_exit_button is False:
            ...
            self.score_box.move_to(self.score_box.spr.x, 290.0, UI_MOVE_SPEED)
            self.start_button.move_to(self.start_button.spr.x, 200.0, UI_MOVE_SPEED)
            self.exit_button.move_to(self.exit_button.spr.x, 200.0, UI_MOVE_SPEED * 0.75)
            ...
            self.title_text.move_to(-200.0, self.title_text.spr.y, UI_MOVE_SPEED)
            self.score_box.move_to(self.score_box.spr.x, core.const.SCREEN_HEIGHT, UI_MOVE_SPEED)
            self.start_button.move_to(self.start_button.spr.x, -self.start_button.spr.image.h, UI_MOVE_SPEED)
            self.exit_button.move_to(self.exit_button.spr.x, -self.exit_button.spr.image.h, UI_MOVE_SPEED)
